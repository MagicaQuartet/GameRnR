...
exports.show = function showRecommendation(req, res) {
  const tagQuery = function createTagQuery(tag) {
	...
	const viewQuery = `CREATE VIEW total_score AS (${totalScoreQuery});`;

	connection.query(viewQuery, function (err, rows) {
		if (err) throw err;
	});

	const resultQuery = 'SELECT g.name AS name, (r.score*g.rating*0.01) AS score FROM all_games AS g INNER JOIN total_score AS r WHERE g.id=r.id AND r.score > 0 ORDER BY score DESC LIMIT 10;';
